...
- CAMERA: отображается live-превью через CameraX `PreviewView` и кнопка съемки; при входе в состояние биндим превью камеры, затем запускается `ACTION_IMAGE_CAPTURE` с записью во временный файл через `FileProvider`. Authority формируется как `${applicationId}.fileprovider` и должен совпадать с `AndroidManifest.xml`.
...
- Instrumentation tests для проверки `FileProvider` находятся в `app/src/androidTest/java`.
